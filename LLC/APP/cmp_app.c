...
void cmp_app_init(void)
{
    ...
    memset((void *)&user_cmp_init, 0x00, sizeof(user_cmp_init));
		...
		// CMP0/CMP1 are used for fast HRPWM fault shutdown.
		// CMP7 is not enabled because PA8 is already HRPWM_OUT0A.
		// CMP8 is not enabled until the DACy source is verified on hardware.
    //User CMP Init
