...
void hrpwm_init(void)
{
    ...
    mpwm.compd   = 3199;
		...
		/*phase1_pwm0 init*/
		phase1_pwm0.period  = LLC_SW_PERIOD_MIN-1;
	
		phase1_pwm0.compa   = LLC_DEADTIME_COUNT - 1;
    phase1_pwm0.compb   = LLC_SW_PERIOD_MIN/2 - 1;
    phase1_pwm0.compc   = LLC_SW_PERIOD_MIN/2 + LLC_DEADTIME_COUNT - 1;
		phase1_pwm0.compd   = LLC_SW_PERIOD_MIN-1;
	
		/*phase1_pwm0 init*/
		phase2_pwm2.period  = LLC_SW_PERIOD_MIN-1;
	
		phase2_pwm2.compa   = LLC_DEADTIME_COUNT - 1;
    phase2_pwm2.compb   = LLC_SW_PERIOD_MIN/2 - 1;
    phase2_pwm2.compc   = LLC_SW_PERIOD_MIN/2 + LLC_DEADTIME_COUNT - 1;
		phase2_pwm2.compd   = LLC_SW_PERIOD_MIN-1;
}
...
void hrpwm_app_config(HRPWM_TypeDef *Instance)
{
    ...
    user_hrpwm_tmr_base_cfg.rep_prd    				= 0;												// Master.MREP Event is triggered after serval repetitions.
    user_hrpwm_tmr_base_cfg.cntr_prd   				= phase1_pwm0.period;
    ...
    LL_HRPWM_TmrBaseCfg(Instance, LLC_PHASE1_PWM0, &user_hrpwm_tmr_base_cfg);		
		...
		user_hrpwm_tmr_base_cfg.cntr_rst_evt     			= HRPWM_SLV2_CNTR_RST_EVT_MST_PWM_PRD;
    ...
    LL_HRPWM_TmrCmpCfg(Instance, HRPWM_MST_PWM, &user_hrpwm_tmr_cmp_cfg);
		...
		user_hrpwm_tmr_cmp_cfg.mst_pwm_upd_en				=false;
    //Slave PWMx Timer Compare Config
    LL_HRPWM_TmrCmpCfg(Instance, LLC_PHASE1_PWM0, &user_hrpwm_tmr_cmp_cfg);
		LL_HRPWM_TmrCmpCfg(Instance, LLC_PHASE2_PWM2, &user_hrpwm_tmr_cmp_cfg);
    ...
    LL_HRPWM_RegUpd_Frc(Instance,LLC_PHASE1_PWM0);
		LL_HRPWM_RegUpd_Frc(Instance,LLC_PHASE2_PWM2);
...
void adc_triger_pwm0_config(HRPWM_TypeDef *Instance)
{
    ...
    user_hrpwm_tmr_base_cfg.resync_mode				= HRPWM_SLV_RESYNC_NEXT_RST_ROLLOVER;
		...
		user_hrpwm_tmr_base_cfg.cntr_rst_evt     		= HRPWM_SLV0_CNTR_RST_EVT_NONE;
		LL_HRPWM_TmrBaseCfg(Instance, USER_ADC_TRIG_PWM1, &user_hrpwm_tmr_base_cfg);
	
		//User HRPWM Common ADC Trigger Config
    user_hrpwm_comm_adctrig_cfg.trig_evt    = HRPWM_COMM_ADC02_TRIG_EVT_PWM1_PRD;
    user_hrpwm_comm_adctrig_cfg.trig_len    = HRPWM_COMM_ADC_TRIG_EVT_LEN_15CLK;
    user_hrpwm_comm_adctrig_cfg.upd_src     = HRPWM_COMM_ADC_TRIG_UPD_SRC_PWM1;
    ...
    LL_HRPWM_Comm_ADCTrigCfg(Instance, HRPWM_ADC_TRIG_NUM_0, &user_hrpwm_comm_adctrig_cfg);
		...
		LL_HRPWM_TmrCmpCfg(Instance, USER_ADC_TRIG_PWM1, &user_hrpwm_tmr_cmp_cfg);

		LL_HRPWM_RegUpd_Frc(Instance,USER_ADC_TRIG_PWM1);
...
void hrpwm_app_start(void)
{
		HRPWM->Master.MCR1  |=
						   ...
						   HRPWM_PWM2_START_BIT|
						   HRPWM_PWM3_START_BIT|
						   HRPWM_PWM4_START_BIT|
						   HRPWM_PWM5_START_BIT|
						   HRPWM_PWM6_START_BIT|
